...
from ..db.queries import get_person_by_id, get_person_facts, get_children, get_parents
...
def validate_all_timelines(min_severity: str = 'warning') -> List[Dict[str, Any]]:
    ...
    return sorted(filtered, key=lambda x: (severity_order.get(x['severity'], 0), x.get('person_name', '')), reverse=True)
